...
export default function AccountsList() {
    ...
    const accountToDelete = useRef(null as {id:number}|null);
    ...
    async function deleteAccount (id:number) {
        accountToDelete.current={id:id};
        Modal.getOrCreateInstance("#deleteAccountModal").show();
    }

    async function confirmDeleteAccount () {
        await AxiosAccounts.delete(accountToDelete?.current?.id as number);
        refreshTable();
    }
    ...
    async function refreshTable () {
        dataTableElem.current?.dt()?.ajax.reload();
    }

    return (
        <div>
            <div>
                <button className="btn btn-lg" onClick={showCreateAccountModal}>Add Account</button>
            </div>
            <DataTable
                ref={dataTableElem}
                columns={columns}
                ...
                slots={{
                    ...
                    3: (data: any, row: Account) => {
                        return <span className="bi bi-trash text-danger" style={{ cursor: "pointer" }} onClick={()=>deleteAccount(row.id as number)}></span>
                    }
                }}
            ...
            <CreateAccountModal onCreate={refreshTable} />
            <DeleteAccountModal confirmDelete={confirmDeleteAccount} />
        </div>
    );
}